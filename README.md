...
| `worktreeCompare.integrationCommitGuard` | `true` | Install a `pre-commit` hook refusing commits made on the integration branch; `git commit --no-verify` overrides. A hook the extension did not write is never replaced |
