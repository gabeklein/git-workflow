...
- **Disabling fast-forwards the base back to reality.** The preview builds on `origin/<base>`, so the *local* base branch is advanced by nothing for as long as preview is on — every PR that lands leaves it one commit further behind. Switching back to it on disable would hand you a working tree missing weeks of landed work, which reads as the preview having reverted it. So the branch returned to is fast-forwarded when it is **strictly behind** origin (local an ancestor of the remote — it can only replay published commits and can drop nothing). A base that is ahead or diverged is left exactly alone: that is the unpushed-work drift above, and resolving it is a decision, not a side effect of disabling.
...
- **`gw-lane absorb` gives the hotfix an exit.** Absorb can only ever aim at the base, which is what makes it the wrong rescue for lane work and the right move for a fix that belongs to the base itself — one found while reading the preview, where the merged lanes are what made it visible. Until it had a CLI it ran from the sidebar alone, so an agent stopped by the commit guard had no applicable exit left and would reach for `--no-verify`; the refusal now names this one, and names the branch the work will land on rather than saying "the base". It takes the rebuild lock for the whole operation (it rewrites the base *and* `reset --hard`s the preview checkout) and queues behind another writer rather than failing, since the holder is usually a rebuild the absorb is still valid after. Exit codes match `rebuild`: 0 moved, 1 ran and refused, 2 could not run. The one refusal that is a question rather than an error is `needs-confirmation` — stray commits that *add* files while lanes are applied, the one shape the replay cannot vet, since an added file carries no diff context and so lands on the base cleanly even when its contents depend on lane code; `--allow-added` is how you say you checked. With the base unchecked-out (the shipped layout — preview is the root switched in place) uncommitted edits have nowhere to land, and it says to commit them first rather than committing on the author's behalf.
...
- **`gw-lane refresh`** stamps a file the editor's `.git` watcher already sees, and the watcher *recognises* the stamp rather than merely waking on it — a plain event refreshes the views that listen for activity, but the worktree rows come from discovery, which otherwise only reruns when membership changes. It exists for the two things a `.git` watch cannot see: a file written into a checkout (nothing under `.git` changed) and a filesystem that drops events, both of which otherwise wait on a 30-second fallback poll. Nothing answers it — a signal is not a request, and there is nobody to ask; if no editor is running it costs one file write.
- **The engine runs without the editor.** Preview settings are resolved into `<git-common-dir>/focus-config` and the extension records how to run the bundled one-shot CLI in `focus-runner`, so `gw-lane rebuild|absorb|add|remove` executes the *same* engine the sidebar runs — the bundle substitutes a `vscode` shim serving those settings, rather than forking the merge rules into a second implementation that would drift. There is deliberately **no resident process**: a queue was built and taken back out, because it serialised requests nothing contended for. Exclusion is what it always was — the rebuild lock — which now **records its holder** (pid, host, and what it is doing), so `gw-lane owner` can answer "would the preview move under me?", and a lock left behind by a process that died on this host is swept rather than waited on. Never swept across hosts: signal 0 says nothing about another machine's process table, and guessing wrong means two writers in a checkout that is about to be `reset --hard`.
- **Headless opt-in**: while preview is on, `<git-common-dir>/gw-lane` is installed — `gw-lane status|check|rebuild|absorb|refresh|owner|add|remove [branch]`. Inclusion is deliberate, and the party doing the work is increasingly an agent that cannot reach a VS Code command, so without a headless surface opt-in just means an empty preview. It speaks the file protocol directly (taking the same `focus-working.lock`, waiting for it rather than failing), so it works with the editor closed. It lives inside `.git`, so it is never committed, is refreshed on every enable, and — unlike a path inside the extension's install directory — does not break when the extension updates.
- **Every rebuild leaves its outcome on disk** in `<git-common-dir>/focus-status`, and `gw-lane status` prints it *before* the membership lists. A refused rebuild never touches the checkout, so the tree stays on the last good chain — which means `focus-applied` can list a lane the preview does not contain, and until now the only place that said so was a tooltip and the output channel, both inside a running editor. The record is plain `key: value` lines (no JSON parser needed in a shell): `state`/`code`/`lane` and a one-line `detail`, `tree` plus `tree-current` for what the checkout actually holds, `resolved:` lines naming any lane whose clashes were auto-resolved and the files whose hunks were dropped, a `next:` naming the move that clears it, and a `tip:` per lane — the shas at record time, which `gw-lane status` compares against the branches now so an already-dealt-with conflict reads as one (`moved since this rebuild ran`). It is a record, never an input: nothing reads it back to decide anything, and it is removed when preview goes off so a dead conflict cannot outlive the preview it described. `gw-lane check` answers the same question as an exit status for anything that wants to gate on it — 0 built and current, 1 a recorded failure (printing the lane and its `next:`), 2 when the record cannot speak for the repo as it stands: nothing recorded, or a lane has moved since, which is what an agent that has just caught its lane up sees instead of its own dealt-with conflict. `status` stays 0 always; printing a report is not a failure.
...
**Lanes** comes first, because it is the **selector**: you click a row there to choose what Focus shows. Selector above detail is the order every other pair in the sidebar uses — Explorer above Outline, repositories above changes — and it is the order you read in, since picking the lane is the step before working in it. It also keeps the preview row, the panel's most consequential state, from sitting below a Focus panel that grows with the diff. Focus gets three quarters of the height (`initialSize`, which behaves like CSS `flex`): Lanes is a list of rows and stops growing, while Focus holds the diff and uses whatever it is given.

(Order and split are both **defaults for a first show**. VS Code remembers your own arrangement, so an existing window keeps whatever you dragged it into — **Reset View Locations** picks up the new ones.)
...
**Working lists every checkout**, the workspace root included. The root used to be hidden while clean (`includeRootCheckout`, now gone), which meant the panel silently omitted whichever branch you had checked out in the repo itself — and with preview off that is the branch every lane is measured against. It is a folder holding a branch, so it is a row; it sorts first and reads `root` (or `root · dirty`). With preview **on** it is the preview instead, and appears as the preview row rather than twice.

**Working also wins over Landed**, so a folder on disk is always reported as one. A landed branch that still had a checkout used to render under Landed, where it looked exactly like a branch that was only a ref — the folder was invisible, which is how stale checkouts piled up. Now:

- a landed checkout that holds **nothing of its own** is removed on sight — clean, unlocked, attached, no ignored files worth keeping, no paused rebase/merge, nothing open in an editor (`autoRemoveLandedWorktrees`). The branch **ref is never deleted**; that is Prune Landed Branches' job, which proves the landing three ways and asks. This only ever gives back a folder `git worktree add` recreates;
- **ignored files that provably survive the removal** do not keep a folder. Ignored files are otherwise a hard stop — the dirty probe cannot see them and `git worktree remove` takes them without complaint — but in a repo that installs per worktree that means every landed lane keeps its folder forever over a `node_modules`. Three things clear one, cheapest and most certain first: a **symlink out of the checkout** (`node_modules -> ../../node_modules` — removing a link cannot touch its target), a file **byte-identical to the root checkout's** at the same path (the `.env` copied in so tests would run: the bytes are still one directory up afterwards), and a match against the **generated-file list** (`node_modules`, `dist`, `.venv`, `target`, `coverage`, `*.log`, …). The first two are evidence, not policy, which is why they are allowed to clear a `.env` that no pattern should ever name. Everything else still blocks: a `.env` the root does not have, a local dump, scratch notes. Whatever a removal took is **named with its reason in Output → Git Workflow**. The list is `worktreeCompare.expendableIgnored`, and it *replaces* the built-in one so an entry can be taken back out; `[]` leaves only the two evidence checks;
