...
| `worktreeCompare.previewBranch` | `preview/{base}` | Branch that opts a checkout into the overlay ({base} = short base name; `focus/working` for script interop) |
...
## Preview worktree (overlay)

Preview lives in the **Preview** group of the Lanes panel. The preview row shows its base, lane count, `+wip`, and any failure state; when off, the group offers an **Enable preview…** row. Enabling asks you to pick:

- **Use this checkout** (default) — the workspace root switches to the preview branch (default `preview/{base}`, e.g. `preview/main`) and becomes the preview surface; disabling switches it back to the branch it was on.
...
- The panel description shows the **base** (`→ main`) every rebuild starts from — current base tip + lanes is exactly what landing those PRs would produce. The panel's title menu has **Rebuild**, **Change Preview Base…** (sets workspace-scoped `previewBaseRef`; empty = `defaultBaseRef`), and **Disable Preview Mode…**.
