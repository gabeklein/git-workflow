...
Integration lives in the **Preview** group of the Lanes panel. The preview row shows its base, lane count, `+wip`, and any failure state; when off, the group offers an **Enable integration…** row. Enabling asks you to pick:
...
- **Lane order is inclusion order**, and lanes can be **dragged** to change it. Lanes merge in the order they were added (`focus-applied` is order-significant, not a sorted set). This matters because the resolver is order-sensitive — union inserts land in merge order, and best-effort resolves same-line clashes toward the *incoming* lane — so under a sorted file, which lane won was decided by branch **name**, and renaming a branch silently changed the preview.
...
- **Reordering**: drag an applied lane onto another to merge it *before* that one, or onto empty space to send it last. Only applied lanes reorder — an unchecked candidate is not in the merge chain, so a position for it would be a position in nothing. The move takes the rebuild lock (the file is shared with the shell script and rewritten by rebuilds retiring landed lanes), and a drop that arrives while a rebuild owns it is dropped rather than queued: the tick that follows renders the order that actually exists.
