...
- **`gw-lane refresh`** stamps a file the editor's `.git` watcher already sees, and the watcher *recognises* the stamp rather than merely waking on it — a plain event refreshes the views that listen for activity, but the worktree rows come from discovery, which otherwise only reruns when membership changes. It exists for the two things a `.git` watch cannot see: a file written into a checkout (nothing under `.git` changed) and a filesystem that drops events, both of which otherwise wait on a 30-second fallback poll. Deliberately not a daemon op — a signal is not a mutation, so there is nothing to serialise and nobody to answer; if no editor is running it costs one file write.
- **One writer: the preview daemon.** Preview mutations run in a single process that everything else — the sidebar and `gw-lane` alike — asks through a request directory in the git common dir. Exclusion stops being something each participant has to get right and becomes a property of the design: requests execute one at a time, in submission order, and "who is driving" is answerable by reading the claim (`gw-lane owner`) rather than by inspecting a lock. **The transport is a directory, not a socket** — `gw-lane` is POSIX sh, and sh cannot speak a unix socket without `nc`/`socat`, so a socket would put a runtime dependency at every call site and hand the editor a private channel agents cannot use, which is the asymmetry this removes. A request is a file moved into place with `rename(2)`: atomic everywhere that matters, inspectable with `ls`, and fine on a network mount. The daemon **exits when idle** (a resident process nobody is using is a liability), **sweeps a claim whose owner died on this host** but never one from another host (we cannot read its process table, and guessing means two writers), and runs the *same* engine the editor runs — the bundle substitutes a `vscode` shim serving the settings the editor recorded in `focus-config`, so a preview built headlessly and one built from the sidebar cannot differ. The editor keeps an in-process fallback for repos where no daemon can be started, and the engine's own lock — which the daemon takes too — is what keeps that correct.
