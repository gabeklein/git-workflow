...
- **derived** ignored files do not keep a folder. Ignored files are otherwise a hard stop — the dirty probe cannot see them and `git worktree remove` takes them without complaint — but in a repo that installs per worktree that means every landed lane keeps its folder forever over a `node_modules`. Installs, build output, caches and test output (`node_modules`, `dist`, `.venv`, `target`, `coverage`, `*.log`, …) are recreated by the command that made them, so they go with the folder and are **named in Output → Git Workflow**. Everything else still blocks: a `.env`, a local dump, scratch notes. The list is `worktreeCompare.expendableIgnored`, and it *replaces* the built-in one so an entry can be taken back out; `[]` restores blocking on any ignored file at all;
